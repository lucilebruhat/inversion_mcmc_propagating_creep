function s = se_cycle_varsli(t,tR,T,mu,Nn,K,zslip,slip,H,zobs,fault_type)
%
% inputs:
...
% - zobs, depth of observation points in km
% output:
% - s, stress rate on the fault
%
% Lucile Bruhat
% June 2018
        ...
        for j = 1:length(zslip)
            for n = 1:Nn % set up for right lateral strike-slip fault!!!!
                Wn(n,i,j)= - 1/(zobs(i)-2*n*H+zslip(j)) + 1/(zobs(i)-2*n*H-zslip(j)) ...
                         + 1/(zobs(i)+2*n*H+zslip(j)) - 1/(zobs(i)+2*n*H-zslip(j));
            end
        end
    end
